...
export default function TextBackgroundColorBtn({textBackgroundColor, onChange}) {
  const [isSelected, setIsSelected] = useState(false);
  const [color, setColor] = useState(textBackgroundColor);
  ...
  return (
    <div className={style.btn}>
      <svg onClick={() => setIsSelected(isSelected => !isSelected)} className="MuiSvgIcon-root jss774" focusable="false" viewBox="0 0 24 24" aria-hidden="true" role="presentation">
        ...
        <path style={{ fill: color }} d="M0 20h24v4H0z"></path>
      </svg>
      { isSelected &&
